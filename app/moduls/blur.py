...
def blureFace_file_ret_json(img,fd_threshold,LOGGER):
...
def blureFace_file(image,fd_threshold,LOGGER):
        ...
        # LOGGER.info(f'face detection started - {time.time()-st}')
        # resp = RetinaFace.detect_faces(image,threshold=fd_threshold)
        # LOGGER.info(f'face detection ended  - {time.time()-st}')
    
        # for identity in resp:
        #     LOGGER.info(f'image blurring started - {time.time()-st} ')
        #     try:
        #         facial_area = resp[identity]['facial_area']   
        #         blurred=cv2.GaussianBlur(image,(71,71),500)
